...
def test_parse_single_line_image_statement():
    ...
    """
    expected_image = 'image target = '
    expected_source = '\'target.png\''
    ...
    assert expected_image == str(decompressed[0].block[0])
    assert expected_source == decompressed[0].block[0].nchildren[0].value
